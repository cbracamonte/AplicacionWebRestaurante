...
public class PedidoDAOPostgre implements IPedidoDAO{
    ...
    public int ingresar(Pedido pedido) throws Exception {
        ...
        String sentenciaSQLlineaDePedido="INSERT INTO lineadepedido(cantidad, condicion, precio, productoid, pedidoid) VALUES (?, ?, ?, ?, ?);";
        try {
            ...
            if(registros_afectados_pedido==1){
                ...
                for(LineaDePedido lineaDePedido: pedido.getLineasDePedido()){
                    ...
                    sentencialineadepedido.setInt(4, lineaDePedido.getProducto().getProductoid());                    
                    ...
                    if(registros_afectados_lineadepedido==1){
                        ...
                        productoDAO.actualizarStock(lineaDePedido.getProducto());
                    }
                    //----------------------------------------------------------------------
                }
    ...
    public Pedido buscar(int pedidoid) throws Exception {
        ...
        sentenciaSQL_lineasdepedido="SELECT ldp.cantidad, ldp.condicion, ldp.precio, "
                + "p.productoid, p.descripcion, p.estado, p.precio, p.stock, p.tipo "
                + " FROM pedido pe "
                + "inner join lineadepedido ldp on pe.pedidoid = ldp.pedidoid "
                + "inner join producto p on ldp.productoid = p.productoid "
                + " where pe.pedidoid = ";
...
//                lineaDePedido.setCantidad(resultado_lineasdepedido.getInt("cantidad"));
//                lineaDePedido.setCondicion(resultado_lineasdepedido.getString("condicion"));
//                lineaDePedido.setPrecio(resultado_lineasdepedido.getDouble("precio"));
                Producto producto= new Producto();
                producto.setProductoid(resultado_lineasdepedido.getInt("productoid"));
                producto.setDescripcion(resultado_lineasdepedido.getString("descripcion"));
                producto.setEstado(resultado_lineasdepedido.getString("estado"));
                producto.setPrecio(resultado_lineasdepedido.getDouble("precio"));
                producto.setStock(resultado_lineasdepedido.getInt("stock"));
                producto.setTipo(resultado_lineasdepedido.getString("tipo"));
//                lineaDePedido.setProducto(producto);
                pedido.agregarLineaDePedido(resultado_lineasdepedido.getInt("cantidad"), resultado_lineasdepedido.getString("condicion"), producto);
            }
        }
    ...
    @Override
    public List<Pedido> ListaPedidosSinCobrar(String estadoPorCobrar) throws SQLException {
        ...
        sentenciaSQL_pedido="SELECT pe.pedidoid, pe.estado, pe.fecha, pe.monto, "
                ...
                + "inner join mesa m on pe.mesaid = m.mesaid "
                + "inner join usuario u on pe.usuarioid = u.usuarioid "
                + " where pe.estado = '" + estadoPorCobrar + "'";
